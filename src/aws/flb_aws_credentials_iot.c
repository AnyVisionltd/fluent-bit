...
/* When Expiration is missing or invalid, refresh after this interval (seconds). */
#define FLB_AWS_IOT_DEFAULT_REFRESH_INTERVAL 1800
                                                 ...
                                                 *generator)
{
    ...
    if (!port_sds) {
        AWS_CREDS_ERROR("Invalid IoT credentials endpoint URL (missing port): %s",
    ...
    if (strcasecmp(protocol, "https") == 0) {
        ...
        use_tls = FLB_FALSE;
        AWS_CREDS_WARN("IoT credentials endpoint '%s' uses http://; the connection "
                       "will not be encrypted and the configured TLS client "
                       "certificate, key and CA will be ignored. Use https:// in "
                       "production.",
                       credentials_endpoint);
    }
    else {
        AWS_CREDS_ERROR("Unsupported scheme '%s' in IoT credentials endpoint "
                        "'%s'; only http:// and https:// are supported.",
                        protocol, credentials_endpoint);
        goto error;
    }
    ...
    AWS_CREDS_DEBUG("Creating upstream connection to %s:%d (tls=%d)", host, port, use_tls);
    upstream = flb_upstream_create(config, host, port,
                                     use_tls ? FLB_IO_TLS : FLB_IO_TCP,
                                     use_tls ? implementation->tls : NULL);
    if (!upstream) {
...
static int iot_credentials_request(struct flb_aws_provider_iot *implementation)
{
    ...
    AWS_CREDS_DEBUG("Client headers count: %zu",
                     (size_t)implementation->client->static_headers_len);
    ...
    if (expiration > 0) {
        ...
        AWS_CREDS_WARN("IoT credentials response did not include a usable "
                       "Expiration; scheduling a refresh in %d seconds.",
                       FLB_AWS_IOT_DEFAULT_REFRESH_INTERVAL);
        implementation->next_refresh = time(NULL) + FLB_AWS_IOT_DEFAULT_REFRESH_INTERVAL;
    }
    flb_http_client_destroy(c);
                                                             ...
                                                             time_t *expiration)
{
    ...
    /*
     * Remove/reset existing value of expiration. If the field is missing
     * or cannot be parsed, the caller applies a default refresh policy.
     */
    *expiration = -1;
